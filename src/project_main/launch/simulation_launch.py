...
def generate_launch_description():
    ...
    # #-------------------------- Spawn sensors and bridge their topics ---------------------------
    for i in range(NUMBER_OF_SENSORS):
        #targets_to_spawn.append(spawn_sdf("resources/active_sensor/active_sensor.sdf", id = i, pos = (randint(-SIMULATION_SIDE, SIMULATION_SIDE), randint(-SIMULATION_SIDE, SIMULATION_SIDE), 0)))
    ...
    targets_to_spawn.append(
            Node(
                ...
                namespace=f"BaseStation",
                name=f"BaseStationController",
                arguments=[
                    f"{NUMBER_OF_SENSORS}"
                ]
            )
